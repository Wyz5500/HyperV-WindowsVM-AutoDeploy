...
$vhdxDirPath = "C:\"
$isoPath = "C:\Win11_24H2_Chinese_Simplified_x64.iso"

#这些是可选设置：
$vhdxSize = 1TB        #动态增长，可以设置的比主机剩余硬盘空间大，不能超过64TB，不能小于64GB。
$switchName = "Default Switch"        #Windows Server主机默认虚拟网络交换机名称可能会有不同。
$index = 1        #用于设置Windows版本（如：家庭版、专业版），可以使用Dism++或者填入"-1"查看索引对应的Windows版本。
$exposeVirtualizationExtensions = $false        #控制是否开启虚拟机的嵌套虚拟化，可选值有"$true"和"$false"
        ...
        try {
            ...
            try {
                ...
                if ($Script:windowsVersion -eq $null) {
                    ...
                    Write-Host (Get-WindowsImage -ImagePath $Script:windowsImage | Select-Object ImageName, ImageIndex)
                    $null = Dismount-DiskImage -ImagePath $isoPath -ErrorAction SilentlyContinue
...
function vhdxCheck {
    ...
    if (Test-Path $vhdxPath) {
        ...
        try {
            ...
            $null = Mount-VHD -Path $vhdxPath -Passthru -ErrorAction Stop    #VHDX挂载后的内部信息，保留用于全局
    ...
    if ($Script:successDeploy -eq 1) {
        try {
            ...
            try {
                ...
                $null = Dismount-DiskImage -ImagePath $isoPath -EA SilentlyContinue
                Remove-Item -Path $vhdxPath -ErrorAction SilentlyContinue
                $Script:successDeploy = 0
            }
        }
...
function installWindows {
    if ($Script:successDeploy -eq 1) {
        ...
        if (Test-Path $Script:windowsImage) {
            Dism /Apply-Image /index:$index /ImageFile:"$Script:windowsImage" /ApplyDir:"$($ntfsDriverLetter):\"
...
# if (
#     $vmName -eq $null -or `
#     $vhdxDirPath -eq $null -or `
#     $vhdxSize -eq $null -or `
#     $cpuCore -eq $null -or `
#     $enableDynamicMemory -eq $null -or `
#     $minMemory -eq $null -or `
#     $maxMemory -eq $null -or `
#     $memory -eq $null -or `
#     $isoPath -eq $null -or `
#     $switchName -eq $null -or `
#     $index -eq $null -or `
#     $xposeVirtualizationExtensions -eq $null
# ) {
#     Write-Warning "任何参数都不能为空"
#     exit 0
# }
...
if (preCheck @preCheckParams) {
    ...
    Write-Host "创建并挂载虚拟硬盘文件……"
    Write-Host "挂载iso镜像文件……"
    ...
    #vhdxCreate      #创建虚拟硬盘，现在是冗余步骤
    vhdxPrepare     #初始化虚拟硬盘，为虚拟硬盘分区
    #isoMount        #挂载iso安装镜像，现在是冗余步骤
